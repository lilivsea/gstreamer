...
gst_h264_parse_init (GstH264Parse * h264parse)
{
  ...
  h264parse->key_event_seq = 0;
  h264parse->keyframepts = GST_CLOCK_TIME_NONE;
}
...
gst_h264_parse_pre_push_frame (GstBaseParse * parse, GstBaseParseFrame * frame)
{
  ...
  keyunit = !GST_BUFFER_FLAG_IS_SET (buffer, GST_BUFFER_FLAG_DELTA_UNIT);
  
  if (keyunit)
  {
	guint64 pts =  GST_BUFFER_TIMESTAMP (buffer);
	if(h264parse-> keyframepts  == GST_CLOCK_TIME_NONE){
  		h264parse->keyframepts = ((pts/1000000 + 500)/2000 *2 +2) *1000000*1000;
		GST_DEBUG_OBJECT(h264parse,"init keyframepts:%"G_GINT64_FORMAT,h264parse->keyframepts);
  	}

	if(h264parse-> keyframepts !=GST_CLOCK_TIME_NONE &&  pts >=  h264parse-> keyframepts)
	{
		GstClockTime  timestamp, stream_time,running_time;      
       		stream_time=running_time=timestamp =GST_BUFFER_TIMESTAMP (buffer);
       		running_time = gst_segment_to_running_time (&parse->segment,
      			GST_FORMAT_TIME, timestamp);
       		event = gst_video_event_new_downstream_force_key_unit
          		(timestamp, stream_time, running_time,TRUE, h264parse->key_event_seq);
       		gst_pad_push_event (GST_BASE_PARSE_SRC_PAD (h264parse), event);	
       		h264parse->key_event_seq ++;
		h264parse ->keyframepts += 2 *1000 *1000000;
                GST_DEBUG_OBJECT(h264parse,"timestamp %" G_GINT64_FORMAT ",keyunit:%d,count:%d",GST_BUFFER_TIMESTAMP (buffer) ,keyunit,h264parse->key_event_seq);
