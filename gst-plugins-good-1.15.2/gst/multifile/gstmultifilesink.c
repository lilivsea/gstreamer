...
gst_multi_file_sink_start (GstBaseSink * bsink)
{
  ...
  sink->segment_pts = GST_CLOCK_TIME_NONE;
...
gst_multi_file_sink_render (GstBaseSink * bsink, GstBuffer * buffer)
{
  ...
  GST_DEBUG_OBJECT (sink, "aggregate GOP: received %s%s unit buffer: "
        "%" GST_PTR_FORMAT,
...
gst_multi_file_sink_event (GstBaseSink * sink, GstEvent * event)
{
  ...
  switch (GST_EVENT_TYPE (event)) {
    case GST_EVENT_CUSTOM_DOWNSTREAM:
    {
      ...
      multifilesink->force_key_unit_count = count;
      multifilesink->segment_pts = stream_time;
      if (multifilesink->file) {
        duration = GST_CLOCK_TIME_NONE;
        offset = offset_end = -1;
        if(multifilesink->segment_pts != GST_CLOCK_TIME_NONE){
		 filename = g_strdup_printf(multifilesink->filename,stream_time);
        }else
        filename = g_strdup_printf (multifilesink->filename,
            multifilesink->index);
        //filename = g_strdup_printf(multifilesink->filename,stream_time); 
      ...
      GST_DEBUG_OBJECT(multifilesink,"segment_pts:%"G_GINT64_FORMAT,multifilesink->segment_pts);
      if (multifilesink->file == NULL) {
        if (!gst_multi_file_sink_open_next_file (multifilesink))
          goto stdio_write_error;
      }
...
gst_multi_file_sink_open_next_file (GstMultiFileSink * multifilesink)
{
  ...
  gst_multi_file_sink_ensure_max_files (multifilesink);
    GST_DEBUG_OBJECT(multifilesink,"lili segment_pts:%"G_GINT64_FORMAT,multifilesink->segment_pts);
  if(multifilesink->segment_pts != GST_CLOCK_TIME_NONE){
      filename = g_strdup_printf(multifilesink->filename,multifilesink->segment_pts);
  }else
      filename = g_strdup_printf (multifilesink->filename,
            multifilesink->index);
