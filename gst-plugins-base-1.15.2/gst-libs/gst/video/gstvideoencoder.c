...
gst_video_encoder_reset (GstVideoEncoder * encoder, gboolean hard)
{
  ...
  priv->frames = NULL;
  encoder->keyframepts = GST_CLOCK_TIME_NONE;
  encoder->keyframeeventcount = 0;
...
gst_video_encoder_push_event (GstVideoEncoder * encoder, GstEvent * event)
{
  switch (GST_EVENT_TYPE (event)) {
    case GST_EVENT_SEGMENT:
    {
      ...
      GST_DEBUG_OBJECT (encoder,"lili new segment event:%" GST_SEGMENT_FORMAT,&encoder->output_segment);
    ...
    case GST_EVENT_CUSTOM_DOWNSTREAM:
    {
      if (gst_video_event_is_force_key_unit (event)) {
        ...
        if (gst_video_event_parse_downstream_force_key_unit (event,
          ...
          GST_DEBUG_OBJECT (encoder,
              " lili force-key-unit event: running-time %" GST_TIME_FORMAT
              ", all_headers %d, count %u,force_key_unit:%p",
              GST_TIME_ARGS (running_time), all_headers, count,encoder->priv->force_key_unit);
        }
        gst_event_unref (event);
        event = NULL;
        ret = TRUE;
    ...
    case GST_EVENT_CUSTOM_UPSTREAM:
    {
      if (gst_video_event_is_force_key_unit (event)) {
        ...
        if (gst_video_event_parse_upstream_force_key_unit (event,
          ...
          GST_DEBUG_OBJECT (encoder,
              "lili force-key-unit event: running-time %" GST_TIME_FORMAT
              ", all_headers %d, count %u,priv->force_key_unit:%p",
              GST_TIME_ARGS (running_time), all_headers, count,encoder->priv->force_key_unit);
        }
        gst_event_unref (event);
        event = NULL;
...
gst_video_encoder_chain (GstPad * pad, GstObject * parent, GstBuffer * buf)
{
  ...
  priv = encoder->priv;
  klass = GST_VIDEO_ENCODER_GET_CLASS (encoder);
  //GST_DEBUG_OBJECT(encoder,"gst_video_encoder_chain\n");
  ...
  //GST_DEBUG_OBJECT(encoder,"pts:%"G_GINT64_FORMAT,pts);
  GST_DEBUG_OBJECT (encoder,
  ...
  if (priv->min_pts != GST_CLOCK_TIME_NONE
    ...
    if (cstart < priv->min_pts) {
      ...
      GST_DEBUG_OBJECT(encoder,"lili time adjustment:%"G_GINT64_FORMAT,priv->time_adjustment);
    }
  }
 ...
 /*
  if(encoder-> keyframepts  == GST_CLOCK_TIME_NONE){
  	encoder->keyframepts = ((pts/1000000 + 500)/2000 *2 +2) *1000000*1000;
	GST_DEBUG_OBJECT(encoder,"init keyframepts:%"G_GINT64_FORMAT,encoder->keyframepts);
  }
 */
  GST_OBJECT_LOCK (encoder);

  if(encoder-> keyframepts  != GST_CLOCK_TIME_NONE && pts >= encoder->keyframepts)
  {
	GST_VIDEO_CODEC_FRAME_SET_FORCE_KEYFRAME (frame);
	GST_VIDEO_CODEC_FRAME_SET_FORCE_KEYFRAME_HEADERS (frame);
        GST_DEBUG_OBJECT(encoder,"set keyframepts:%"G_GINT64_FORMAT,encoder->keyframepts);
        encoder-> keyframepts += 2 *1000 *1000000;
        if(GST_VIDEO_CODEC_FRAME_IS_FORCE_KEYFRAME(frame) )
        {
        GST_DEBUG_OBJECT(encoder,"this frame is keyframe 2");
        }
  ...
  if (priv->force_key_unit) {
    ...
    running_time =
        ...
        cstart);
    GST_DEBUG_OBJECT (encoder,"lili force_key_unit %p", priv->force_key_unit);
    for (l = priv->force_key_unit; l; l = l->next) {
      ForcedKeyUnitEvent *tmp = l->data;
      GST_DEBUG_OBJECT (encoder,"tmp:%p,pending:%d,tmp->running_time:%" GST_TIME_FORMAT",running_time:%" GST_TIME_FORMAT,tmp,
		tmp->pending,GST_TIME_ARGS (tmp->running_time),GST_TIME_ARGS (running_time));
      /* Skip pending keyunits */
      if (tmp->pending){
   	GST_DEBUG_OBJECT (encoder,"tmp->pending");
        continue;
      }

      /* Simple case, keyunit ASAP */
      if (tmp->running_time == GST_CLOCK_TIME_NONE) {
	GST_DEBUG_OBJECT (encoder,"tmp->running_time == GST_CLOCK_TIME_NONE");
        fevt = tmp;
        break;
      }
      ...
      if (tmp->running_time <= running_time) {
      	GST_DEBUG_OBJECT (encoder,"tmp->running_time :%" GST_TIME_FORMAT", running_time:%" GST_TIME_FORMAT,
			GST_TIME_ARGS (tmp->running_time),GST_TIME_ARGS(running_time));
       fevt = tmp;
        break;
      }
    }
     GST_DEBUG_OBJECT (encoder,"lili fevt:%p",fevt);
    if (fevt) {
  ...
  GST_DEBUG_OBJECT (encoder, "passing frame pfn %d to subclass",
  ...
  if(GST_VIDEO_CODEC_FRAME_IS_FORCE_KEYFRAME(frame) )
  {
	GST_DEBUG_OBJECT(encoder,"this frame is keyframe,pts:%"G_GINT64_FORMAT,frame->pts);
  }
    ...
    GstVideoCodecFrame * frame)
{
  ...
  priv->processed++;
  // GST_DEBUG_OBJECT (encoder,"force_key_unit :%p,lili keyframe:%d",priv->force_key_unit,GST_VIDEO_CODEC_FRAME_IS_SYNC_POINT (frame));
  if (GST_VIDEO_CODEC_FRAME_IS_SYNC_POINT (frame) && priv->force_key_unit) {
    ...
    if (fevt) {
      ...
      GST_DEBUG_OBJECT (encoder,
          ...
          GST_TIME_ARGS (running_time), GST_TIME_ARGS(stream_time), fevt->all_headers, fevt->count);
   ...
   if(encoder->keyframepts != GST_CLOCK_TIME_NONE )
   {
	...
	GstEvent *ev;
        ...
        stream_time =
          ...
          frame->pts);
        ev = gst_video_event_new_downstream_force_key_unit
          (frame->pts, stream_time, running_time,TRUE, encoder->keyframeeventcount);
        GST_DEBUG_OBJECT (encoder,
          "lili Forced key unit: running-time %" GST_TIME_FORMAT",stream_time %" GST_TIME_FORMAT",count %ld", GST_TIME_ARGS (running_time),
		 GST_TIME_ARGS (stream_time), encoder->keyframeeventcount);

        gst_video_encoder_push_event (encoder, ev);
        GST_OBJECT_LOCK (encoder);
        encoder->keyframeeventcount ++;
        GST_OBJECT_UNLOCK (encoder);
   }
...
gst_video_encoder_set_min_pts (GstVideoEncoder * encoder, GstClockTime min_pts)
{
  ...
  GST_DEBUG_OBJECT(encoder,"lili set min pts  %" GST_TIME_FORMAT,
      GST_TIME_ARGS(min_pts) );
}
